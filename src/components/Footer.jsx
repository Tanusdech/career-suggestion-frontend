...
export default function Footer({ onToggle }) {
  ...
  return (
    <footer className="bg-background text-neutral-500 text-xs sm:text-sm text-center py-2 select-none border-t border-neutral-200 relative">
      ...
      <div className="text-[10px] sm:text-xs text-gray-400 leading-tight flex justify-center items-center gap-1 relative">
        &copy; {new Date().getFullYear()} Developed by{' '}
        <button
          type="button"
          onClick={() => setOpenQR((prev) => !prev)}
          className="text-primary-500 font-semibold hover:underline focus:outline-none"
        >
          Tanusdech Monplub
        </button>
        All rights reserved.
